...
use crate::yaml::{ptrdiff_t, size_t, yaml_char_t};
use crate::{
    libc, yaml_free, yaml_malloc, yaml_mark_t, yaml_parser_t, yaml_parser_update_buffer,
    yaml_queue_extend, yaml_scalar_style_t, yaml_simple_key_t, yaml_stack_extend,
    yaml_string_extend, yaml_string_join, yaml_string_t, yaml_token_delete, yaml_token_t,
            ...
            _ => {
                if (string.end).c_offset_from(string.start) as libc::c_long as size_t <= length {
                    ...
                    if length > 1_u64 {
                        ...
                        string.pointer =
                            (string.pointer).wrapping_offset(length.wrapping_sub(1_u64) as isize);
                    }
                    if if (*parser).unread >= 1_u64 {
                        1_i32
                    } else {
