...
unsafe fn yaml_parser_parse_document_start(
        ...
        != YAML_STREAM_END_TOKEN as libc::c_int as libc::c_uint
    {
        ...
        while tag_directives.start != tag_directives.end {
            yaml_free(
                (*(tag_directives.end).c_offset(-(1 as libc::c_int) as isize)).handle
                    as *mut libc::c_void,
            );
            yaml_free(
                (*(tag_directives.end).c_offset(-(1 as libc::c_int) as isize)).prefix
                    as *mut libc::c_void,
            );
            tag_directives.end = (tag_directives.end).c_offset(-1);
        }
...
unsafe fn yaml_parser_parse_document_content(
        ...
        || (*token).type_0 as libc::c_uint == YAML_STREAM_END_TOKEN as libc::c_int as libc::c_uint
    {
        ...
        *fresh21 = (*fresh21).c_offset(-1);
        ...
        yaml_parser_parse_node(parser, event, 1 as libc::c_int, 0 as libc::c_int)
    }
}
                        ...
                        loop {
                            ...
                            ) == 0 as libc::c_int
                            {
                                ...
                                if tag.is_null() {
                                    ...
                                    memcpy(
                                        tag.c_offset(prefix_len as isize) as *mut libc::c_void,
                                        tag_suffix as *const libc::c_void,
                                        suffix_len,
                                    );
                                    *tag.c_offset(prefix_len.wrapping_add(suffix_len) as isize) =
...
unsafe fn yaml_parser_parse_block_sequence_entry(
    ...
    if first != 0 {
        ...
        if if (*parser).marks.top != (*parser).marks.end
            || yaml_stack_extend(
                ptr::addr_of_mut!((*parser).marks.start) as *mut *mut libc::c_void,
                ptr::addr_of_mut!((*parser).marks.top) as *mut *mut libc::c_void,
                ptr::addr_of_mut!((*parser).marks.end) as *mut *mut libc::c_void,
            ) != 0
        {
    ...
    if (*token).type_0 as libc::c_uint == YAML_KEY_TOKEN as libc::c_int as libc::c_uint {
        ...
        *fresh80 = (*fresh80).c_offset(1);
                ...
                != YAML_BLOCK_END_TOKEN as libc::c_int as libc::c_uint
        {
            ...
            yaml_parser_parse_node(parser, event, 1 as libc::c_int, 1 as libc::c_int)
        } else {
...
unsafe fn yaml_parser_parse_flow_sequence_entry_mapping_key(
            ...
            != YAML_FLOW_SEQUENCE_END_TOKEN as libc::c_int as libc::c_uint
    {
        ...
        yaml_parser_parse_node(parser, event, 0 as libc::c_int, 0 as libc::c_int)
    } else {
        let mark: yaml_mark_t = (*token).end_mark;
        (*parser).token_available = 0 as libc::c_int;
...
unsafe fn yaml_parser_process_empty_scalar(
    ...
    *value.c_offset(0 as libc::c_int as isize) = '\0' as i32 as yaml_char_t;
    memset(
        event as *mut libc::c_void,
        0 as libc::c_int,
        mem::size_of::<yaml_event_t>() as libc::c_ulong,
...
unsafe fn yaml_parser_process_directives(
    ...
    if !(if !(tag_directives.start).is_null() {
        ...
        tag_directives.end = (tag_directives.start).c_offset(16 as libc::c_int as isize);
        1 as libc::c_int
    } else {
        (*parser).error = YAML_MEMORY_ERROR;
        0 as libc::c_int
            ...
            loop {
                    ...
                    == YAML_TAG_DIRECTIVE_TOKEN as libc::c_int as libc::c_uint
                {
                    ...
                    if if tag_directives.top != tag_directives.end
                        || yaml_stack_extend(
                            ptr::addr_of_mut!(tag_directives.start) as *mut *mut libc::c_void,
                            ptr::addr_of_mut!(tag_directives.top) as *mut *mut libc::c_void,
                            ptr::addr_of_mut!(tag_directives.end) as *mut *mut libc::c_void,
                        ) != 0
                    {
                        let fresh141 = tag_directives.top;
                        tag_directives.top = (tag_directives.top).c_offset(1);
                        *fresh141 = value;
                        1 as libc::c_int
                    } else {
                        (*parser).error = YAML_MEMORY_ERROR;
            ...
            match current_block {
                ...
                _ => {
                    ...
                    loop {
                        ...
                        default_tag_directive = default_tag_directive.c_offset(1);
                    }
                    match current_block {
                        17143798186130252483 => {}
                        _ => {
...
unsafe fn yaml_parser_append_tag_directive(
    ...
    if (copy.handle).is_null() || (copy.prefix).is_null() {
        ...
        || yaml_stack_extend(
            ptr::addr_of_mut!((*parser).tag_directives.start) as *mut *mut libc::c_void,
            ptr::addr_of_mut!((*parser).tag_directives.top) as *mut *mut libc::c_void,
            ptr::addr_of_mut!((*parser).tag_directives.end) as *mut *mut libc::c_void,
        ) != 0
    {
